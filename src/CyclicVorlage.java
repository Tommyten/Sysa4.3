public class CyclicVorlage {
	...
	static final int MAXTIME = Integer.MAX_VALUE;
	...
	static int neighbours(int i, int j) {
		...
		if (S[il][j ] == actValue)
			neighbors++;
		if (S[i ][jl] == actValue) neighbors++;
		if (S[ir][j ] == actValue) neighbors++;
		if (S[i ][jr] == actValue) neighbors++;

		if (S[il][jl] == actValue) neighbors++;
		if (S[ir][jl] == actValue) neighbors++;
		if (S[ir][jr] == actValue) neighbors++;
		if (S[il][jr] == actValue) neighbors++;

		if(neighbors >= 2) newValue = actValue;
